...
do
    ...
    if [[ "$redirect" == "true" ]]; then
        ${command_sql} | tee "${file_path}${file_name}.sql"    # Use tee for output, redirecting to file and displaying on the command line
        ${command_txt} | tee "${file_path}${file_name}.txt"    # Use tee for output, redirecting to file and displaying on the command line
    else
        ${command_sql} 
        ${command_txt}
    fi
